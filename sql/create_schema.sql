...
BEGIN
    SELECT accounts.type INTO acc_type FROM accounts WHERE :new.assigned_worker_username = accounts.account_username;
...
CREATE OR REPLACE TRIGGER check_client_assigned_to_order
    BEFORE INSERT OR UPDATE OF client_username
    ON orders
    FOR EACH ROW
DECLARE
    acc_type VARCHAR(255);
...
BEGIN
    SELECT COUNT(*) INTO unfinished_jobs FROM jobs WHERE jobs.order_id = :new.order_id AND jobs.status <> 'COMPLETED';
